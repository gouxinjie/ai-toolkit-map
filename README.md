...
纯前端单页应用，**无需构建、无需后端**。由于数据通过 `fetch` 动态加载 `aihot_data.json`，请通过**本地静态服务器**方式访问（浏览器对 `file://` 协议的 `fetch` 有跨域限制，直接双击打开会提示数据加载失败）。
...
无需安装任何依赖，推荐用本地静态服务器访问：

```bash
# 推荐：本地静态服务器（避免 file:// 下的 fetch 跨域限制）
...
> ⚠️ **不建议直接双击 `index.html` 打开**：页面通过 `fetch` 加载 `aihot_data.json`，而浏览器出于安全策略会拦截 `file://` 协议下的 `fetch` 请求，导致数据加载失败、页面空白。
