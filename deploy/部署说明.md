...
通过 GitHub Actions，在每次推送（push）到 `master` 分支时，自动将站点部署到阿里云 ECS 的 `/var/www/ai-toolkit-map` 目录。

- 域名：`aihot.gouxinjie.com`
- 协议：HTTP（80 端口）
- ECS 上 Nginx 监听 80 端口，与其他项目通过 `server_name` 区分域名，共存互不影响
...
| 文件 | 作用 |
|------|------|
| `.github/workflows/deploy.yml` | GitHub Actions 工作流：通过 SSH 将代码同步到 ECS |
| `deploy/server_aihot.conf` | Nginx 站点配置（复制到 ECS 上使用） |

---

## 二、在 GitHub 仓库配置密钥（Secrets）

进入仓库 → `Settings`（设置） → `Secrets and variables`（密钥和变量） → `Actions` → `New repository secret`（新建仓库密钥），添加以下 4 个密钥：
...
- **自动触发**：任何推送到 `master` 分支都会自动部署。
- **手动触发**：在 Actions（操作）页面选中该工作流 → `Run workflow`（运行工作流）。
...
在 GitHub 的 `Actions`（操作）标签页查看运行日志，确认 `Deploy to Aliyun ECS` 步骤显示成功（绿色对勾）。
...
| 问题 | 排查方向 |
|------|---------|
| 部署失败：`Host key verification failed`（主机密钥验证失败） | 工作流中已加 `-o StrictHostKeyChecking=no`，若仍报错请检查网络/端口是否可达 |
| 部署失败：`Permission denied (publickey)`（公钥认证被拒） | 见上文「私钥换行符问题」排查步骤 |
| 域名打不开 | 检查 DNS 解析、安全组 80 端口、Nginx 配置是否已 reload |
